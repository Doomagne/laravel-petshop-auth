...
<div style="max-width: 1100px; margin: 2rem auto; padding: 1.5rem;">
    <div style="display:flex; align-items:center; justify-content:space-between; gap:.75rem; flex-wrap:wrap; margin-bottom:.75rem;">
        <h1 style="font-size:1.6rem; font-weight:700; color:#111827; margin:0;">Dogs</h1>
        <a href="{{ route('admin.dashboard') }}" style="color:#2563eb; text-decoration:underline;">← Back to Dashboard</a>
    </div>
    <div style="display:flex; gap:.5rem; flex-wrap:wrap; margin-bottom:1rem;">
        <a href="{{ route('admin.dogs.create') }}" style="background:#2563eb; color:white; padding:0.55rem 1rem; border-radius:8px; text-decoration:none; font-weight:600;">+ Add Dog</a>
    </div>
                ...
                <tbody>
                    @forelse($dogs as $dog)
                        <tr style="border-top:1px solid #e5e7eb;">
                            <td style="padding:.6rem;">{{ $dog->name }}</td>
                            <td style="padding:.6rem;">{{ $dog->breed_label }}</td>
                            <td style="padding:.6rem;">{{ $dog->age_label }}</td>
                            <td style="padding:.6rem;">{{ ucfirst($dog->status) }}</td>
                            <td style="padding:.6rem; display:flex; gap:.5rem; flex-wrap:wrap;">
                                <a href="{{ route('admin.dogs.show', $dog) }}" style="color:#2563eb;">View</a>
                                <a href="{{ route('admin.dogs.edit', $dog) }}" style="color:#f59e0b;">Edit</a>
                                <form action="{{ route('admin.dogs.destroy', $dog) }}" method="POST" onsubmit="return confirm('Delete this dog?');" style="margin:0;">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="5" style="padding:1rem; text-align:center; color:#6b7280;">No dogs yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
